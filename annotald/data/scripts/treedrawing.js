...
function handleMouseWheel(e, delta) {
  if (e.shiftKey && startnode) {
    var nextNode;
    if (delta < 0) {
      // negative means scroll down, counterintuitively
...
function showContextMenu(e) {
  ...
  conr.height('auto');
  conrr.height('auto');
  var h = _.max([conl, conr, conrr], function (x) {
    return x.height();
  });
  conl.height(h);
  conr.height(h);
...
function updateMetadataEditor() {
  ...
  var addButtonHtml =
    '<input type="button" id="addMetadataButton" ' + 'value="Add" />';
  $('#metadata').html(
    dictionaryToForm(getMetadata($(startnode))) + addButtonHtml
  );
  $('#metadata')
    .find('.metadataField')
    .change(saveMetadata)
...
function addMetadataDialog() {
  ...
  var html =
    'New Name: <input type="text" id="metadataNewName" value="NEW" />' +
...
function splitWord() {
  ...
  var html =
    "Enter an at-sign at the place to split the word: \
<input type='text' id='splitWordInput' value='" +
    origWord +
...
function editComment() {
  ...
  showDialogBox(
    ...
    '<textarea id="commentEditBox">' +
      commentText +
      '</textarea><div id="commentTypes">' +
      commentTypeCheckboxes +
      '</div><div id="dialogButtons">' +
      '<input type="button"' +
  ...
  $('#commentEditBox')
    .focus()
    .get(0)
    .setSelectionRange(commentText.length, commentText.length);
  function editCommentDone(change) {
    if (change) {
      ...
      if (/_|\n|:|\}|\{|\(|\)/.test(newText)) {
        ...
        alert(
          'illegal characters in comment: illegal characters are' +
            ' _, :, {}, (), and newline'
        );
        // hideDialogBox();
...
function leafEditorHtml(label, word, lemma) {
  ...
  if (lemma) {
    editorHtml +=
      "<input id='leaflemmabox' class='labeledit' " +
      "type='text' value='" +
      lemma +
      "' />";
  }
...
function displayRename() {
  ...
  if ($(startnode).children('.wnode').size() > 0) {
    ...
    $('#leafphrasebox,#leaftextbox,#leaflemmabox')
          ...
          if (isLeafNode) {
            if (typeof testValidLeafLabel !== 'undefined') {
              if (!testValidLeafLabel(newlabel)) {
                displayWarning("Not a valid leaf label: '" + newlabel + "'.");
                return;
              }
            }
          } else {
            if (typeof testValidPhraseLabel !== 'undefined') {
              if (!testValidPhraseLabel(newlabel)) {
                displayWarning("Not a valid phrase label: '" + newlabel + "'.");
                return;
              }
            }
          }
    ...
    var editor = $(
      "<input id='labelbox' class='labeledit' " +
        "type='text' value='" +
        label +
        "' />"
    );
      ...
      .keydown(function (event) {
        ...
        if (event.keyCode == 13) {
          ...
          if (typeof testValidPhraseLabel !== 'undefined') {
            if (
              !(
                testValidPhraseLabel(newphrase) ||
                (typeof testValidLeafLabel !== 'undefined' &&
                  isWordLevelConj &&
                  testValidLeafLabel(newphrase))
              )
            ) {
              displayWarning("Not a valid phrase label: '" + newphrase + "'.");
              return;
            }
          }
...
function editLemma() {
  ...
  var editor = $(
    "<span id='leafeditor' class='wnode'><input " +
  ...
  $('#leaflemmabox').keydown(function (event) {
    ...
    if (event.keyCode == 13) {
      ...
      $('#leafeditor').replaceWith(
        "<span class='lemma'>-" + newlemma + '</span>'
      );
      postChange();
    }
    // TODO: escape
 ...
 * @constant
 */
var searchornodehtml =
  "<div class='searchnode searchornode'>" +
  "<div class='searchadddelbuttons'>" +
 ...
 * @constant
 */
var searchdeepnodehtml =
  "<div class='searchnode searchdeepnode'>" +
  "<div class='searchadddelbuttons'>" +
  "<input type='button' class='searchdelnodebut' value='-' />" +
  '</div>' +
 ...
 * @constant
 */
var searchprecnodehtml =
  "<div class='searchnode searchprecnode'>" +
  "<div class='searchadddelbuttons'>" +
  "<input type='button' class='searchdelnodebut' value='-' />" +
  '</div>' +
 ...
 * @constant
 */
var addsearchnodehtml =
  "<div class='newsearchnode'>" +
 ...
 */
var searchhtml =
  "<div id='searchnodes' class='searchnode'><input type='hidden' " +
  "class='searchtype' value='Root' />" +
  searchnodehtml +
  '</div>';
...
function searchNodePostAdd(node) {
  ...
  rejiggerSearchSiblingAdd();
  var nodeToFocus =
    (node && node.find('.searchtext')) || $('.searchtext').first();
  nodeToFocus.focus();
}
...
function interpretSearchNode(node, target, options) {
  ...
  if (searchtype == 'Label') {
    ...
    rx = RegExp('^' + $(node).children('.searchtext').val(), 'i');
    hasMatch =
      $(target).children('.wnode').length == 1 &&
    ...
    rx = RegExp('^' + $(node).children('.searchtext').val(), 'i');
    hasMatch = hasLemma($(target)) && rx.test(getLemma($(target)));
    if (!hasMatch) {
      return undefined;
    }
...
function moveNode(parent) {
  ...
  var nodeMoved;
  if (
    !isPossibleTarget(parent) || // can't move under a tag node
    ...
    if ($(startnode).parent().children().first().is(startnode)) {
      ...
      $(startnode).insertBefore(
        $(parent).children().filter($(startnode).parents())
      );
...
function moveNodes(parent) {
  ...
  if (startnode.parentNode == endnode.parentNode) {
    ...
    $(startnode)
      .add($(startnode).nextUntil(endnode))
      .add(endnode)
...
function makeLeaf(before, label, word, target) {
  ...
  if (endnode) {
    ...
    if (startRoot == endRoot) {
      ...
      // abort if selecting from different tokens
      undoAbortTransaction();
      return;
    }
  }
  ...
  var newleaf =
    "<div class='snode " + label + "'>" + label + "<span class='wnode'>" + word;
  if (lemma) {
  ...
  if (startnode && !endnode) {
    ...
    if (getIndex($(startnode)) > 0 && getIndex($(endnode)) > 0) {
      ...
      if (getIndex($(startnode)) == getIndex($(endnode))) {
        ...
        var types =
          '' + getIndexType($(startnode)) + '' + getIndexType($(endnode));
        // remove it
...
function save(e, extraArgs) {
  ...
  if (
    document.getElementById('leafphrasebox') ||
  ...
  if (!saveInProgress) {
    ...
    setTimeout(function () {
      ...
      $.post('/doSave', extraArgs, saveHandler).error(function () {
        lastsavedstate = '';
        saveInProgress = false;
        displayError('Save failed, could not ' + 'communicate with server!');
      });
      unAutoIdle();
      lastsavedstate = $('#editpane').html();
    }, 0);
  ...
  return $.ajax('/advanceTree', {
    ...
    success: function (res) {
      if (res.result == 'failure') {
        ...
        displayTreeIndex(
          'Editing tree #' + currentIndex + ' out of ' + res['totalTrees']
        );
      }
    },
    dataType: 'json',
    type: 'POST',
    data: { trees: theTrees, find: find, offset: offset },
  });
}
...
function goToTree() {
  ...
  var html =
    "Enter the index of the tree you'd like to jump to: \
      ...
      var key =
        ...
        theFn =
          ...
          args
            ...
            .join(', ') +
          ')';
      logEvent('keypress', { key: key, fn: theFn });
    });

    addClickHook(function (button) {
      logEvent('mouse-click', { button: button });
    });
...
function quitServer(e, force) {
  ...
  if (!force && $('#editpane').html() != lastsavedstate) {
    displayError(
      "Cannot exit, unsaved changes exist.  <a href='#' " +
        "onclick='quitServer(null, true);return false;'>Force</a>"
    );
  } else {
    if (window.confirm('Are you sure you want to exit?')) {
      $('#loading-spinner').fadeIn();
      $.post('/doExit', function (response) {
...
function basesAndDashes(bases, dashes) {
  function _basesAndDashes(string) {
    ...
    return (
      bases.indexOf(b) > -1 &&
      _.all(spl, function (x) {
        return dashes.indexOf(x) > -1;
      })
    );
  }
...
function removeIndex(node) {
  ...
  setLabelFn(
    node,
    label.substr(0, Math.max(label.lastIndexOf('-'), label.lastIndexOf('='))),
    true
  );
}
...
function updateCssClass(node, oldlabel) {
  ...
  if (oldlabel) {
    ...
    oldlabel = _.find(oldlabel, function (s) {
      return /[A-Z-]/.match(s);
    });
  }
...
function resetLabelClasses(alertOnError) {
  var nodes = $('.snode').each(function () {
    ...
    if (alertOnError) {
      ...
      classes.indexOf(label) >= 0 && classes.splice(classes.indexOf(label), 1);
      if (classes.length > 0) {
        alert(
          "Spurious classes '" +
            classes.join() +
